...
function renderTrip() {
  ...
  let html = `<div class="trip-toolbar">
    ...
    <button class="btn btn-ghost btn-small" data-act="manage-visas">🛂 证件${visaAlertN ? `<span class="visa-dot">${visaAlertN}</span>` : ''}</button>
  </div>
  ...
  $('[data-act="manage-visas"]')?.addEventListener('click', () => openVisaManager());
...
function openVisaManager() {
  ...
  openModal({
    ...
    body: `
      <div class="visa-section">
        <div class="visa-sec-head">有效期证件 <span class="muted small">${dated.length} 项</span></div>
        ${dated.length
  ...
  setTimeout(() => {
    $$('.visa-card[data-vid]', $('#modal-root')).forEach(card => {
      card.onclick = () => {
        const v = State.visas.find(x => x.id === card.dataset.vid);
        if (!v) return;
        closeModal();
        openVisaModal(v);
      };
...
function openVisaModal(existing, defaultType) {
  ...
  const extraButtons = existing ? [
    { label: '删除证件', danger: true, onClick: () => {
      ...
      persistVisas(); persistTasks();
      closeModal(); renderAll(); openVisaManager(); toast('已删除');
    }},
  ] : [];
